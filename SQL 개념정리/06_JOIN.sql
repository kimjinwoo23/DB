 ...
 - 관계형 데이터베이스에서 SQL 이용해 테이블간 관계를 맺는 방법
  ...
  - 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어
      원하는 정보를 테이블에서 조회하려면 한 개 이상의 테이블에서
        데이터를 읽어와야 하는 경우가 많음
        이 때, 테이블관 관계를 맺기 위한 연결고리 역할이 필요한데
...
/*OUTER JOIN 외부조인
두 테이블의 지정하는 컬럼값이 일치하지않는 행도 
조인에 일치시킴


*/

//OUTER JOIN 과 비교할 INNER JOIN 문 
SELEcT EMP_NAME,DEPT_TITLE
FROM EMPLOYEE 
JOIN department ON ( DEPT_CODE=DEPT_ID)

--1)RIGHT (OUTER) JOIN
--합치기에 사용 한 두테이블중에서 오른쪽에 작성된 테이블의 
--컬럼수를 기준으로 JOIN
 ...
 --모든행이 최종결과에 포함하도록하는 JOIN
 
 --ORACLE
 SELECT EMP_NAME,DEPT_TITLE
 FROM EMPLOYEE,department
 WHERE DEPT_CODE(+) = DEPT_ID;
 --DEPT_CODE와 DEPT_ID 값이 일치하지않아도 
 --오른쪽에 있는 컬럼의 수에 맞게 표시를 넣어라
 
------------------------------------------------------

--1)LEFT (OUTER) JOIN
--합치기에 사용 한 두테이블중에서 왼쪽에 작성된 테이블의 
--컬럼수를 기준으로 JOIN
 ...
 WHERE DEPT_CODE = DEPT_ID(+);
 --DEPT_CODE와 DEPT_ID 값이 일치하지않아도 
 --왼쪽에 있는 컬럼의 수에 맞게 표시를 넣어라

-------------------------------------------


--1)FULL (OUTER) JOIN
--합치기에 사용한 두 테이블이 가진 모든행을 결과에포함
 --오라클에서는 FULL OUTER JOIN구문사용불가
